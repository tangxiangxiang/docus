...
function assetError(
  code: 'ASSET_PENDING_INVALID' | 'ASSET_UPLOAD_FAILED' | 'ASSET_ID_CONFLICT' | 'ASSET_MISSING' | 'ASSET_RESOLVE_FAILED',
  message: string,
  cause?: unknown,
): BoardAssetError {
  return new BoardAssetError(code, message, 500, { cause })
}
...
export function createBoardAssetSession(options: BoardAssetSessionOptions): BoardAssetSession {
  ...
  function ensureRecordForRuntimeAsset(asset: BoardRuntimeAsset): SessionAssetRecord {
    ...
    if (!isBlob(asset.blob) || asset.blob.size <= 0 || asset.blob.size > maxBytes) {
      throw assetError('ASSET_PENDING_INVALID', `Runtime image ${asset.engineFileId} exceeds the Board asset limit or has no data.`)
    }
    const mimeType = normalizeMimeType(asset.mimeType)
    if (!isBoardAssetMimeType(mimeType)) {
      throw assetError('ASSET_PENDING_INVALID', `Runtime image ${asset.engineFileId} uses an unsupported MIME type.`)
    }
