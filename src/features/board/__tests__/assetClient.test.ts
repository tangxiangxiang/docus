...
import { BoardAssetError, fetchAssetBlob, uploadAsset } from '../assetClient'
...
describe('Board asset client', () => {
  ...
  it('rejects unsupported MIME and oversized blobs before making a request', async () => {
    await expect(uploadAsset(assetId, 'image/svg+xml', new Blob(['svg']))).rejects.toMatchObject({ code: 'ASSET_UPLOAD_FAILED', status: 415 })
    await expect(uploadAsset(assetId, 'image/png', new Blob([new Uint8Array(20 * 1024 * 1024 + 1)]))).rejects.toMatchObject({ code: 'ASSET_UPLOAD_FAILED', status: 413 })
