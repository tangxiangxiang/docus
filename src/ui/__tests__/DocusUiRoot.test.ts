...
import DocusUiRoot from '../DocusUiRoot.vue'
import { useI18n } from '../../composables/useI18n'
import { useTheme } from '../../composables/useTheme'
import { docusNaivePrimaryColors } from '../naiveTheme'
...
function mountRoot(): VueWrapper {
  const wrapper = mount(DocusUiRoot, { attachTo: document.body })
...
describe('DocusUiRoot', () => {
  ...
  it('bridges Docus theme state to Naive without remounting App', async () => {
    const wrapper = mountRoot()
    const provider = wrapper.findComponent(NConfigProvider)
    await nextTick()

    expect(provider.props('theme')).toBe(null)
    expect(primaryColor(wrapper)).toBe(docusNaivePrimaryColors.light)
    expect(wrapper.get('[data-testid="app-mount-count"]').text()).toBe('1')

    useTheme().set('dark')
    await nextTick()
    expect(provider.props('theme')).toBe(darkTheme)
    expect(primaryColor(wrapper)).toBe(docusNaivePrimaryColors.dark)
    expect(wrapper.get('[data-testid="app-mount-count"]').text()).toBe('1')
  ...
  it('bridges Docus locale to Naive locale and dateLocale without remounting App', async () => {
    const wrapper = mountRoot()
    const provider = wrapper.findComponent(NConfigProvider)
    await nextTick()
  ...
  it('remains installable at the router application boot boundary', async () => {
    ...
    const app = (await import('vue')).createApp(DocusUiRoot)
    app.use(router)
    app.mount(host)
    ...
    expect(mainSource).toContain("import DocusUiRoot from './ui/DocusUiRoot.vue'")
    expect(mainSource).toMatch(/createApp\(DocusUiRoot\)\.use\(router\)\.mount\('#app'\)/)
  })
})
