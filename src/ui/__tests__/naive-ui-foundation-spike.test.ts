...
function installDocusTokens(): void {
...
describe('Naive UI foundation compatibility spike', () => {
  ...
  it('preserves Tabler currentColor and lets consumers control icon density', () => {
    ...
    expect(standalone.attributes('style')).toContain('color: var(--docus-accent)')
...
const values = JSON.parse(process.env.DOCUS_SPIKE_DATES)
...
`
    ...
    for (const zone of zones) {
      const result = spawnSync(process.execPath, ['--input-type=module', '-e', script], {
        cwd: process.cwd(),
        env: { ...process.env, TZ: zone, DOCUS_SPIKE_DATES: JSON.stringify(dates) },
        encoding: 'utf8',
      })
  ...
  it('tree-shakes a small Tabler entry without bundling the whole catalog', async () => {
    const outputDir = await mkdtemp(join(tmpdir(), 'docus-tabler-spike-'))
    try {
      await build({
        root: process.cwd(),
        configFile: false,
