...
import { createDocusNaiveThemeOverrides } from '../../ui/naiveTheme'
...
function installBrowserApiShims(): void {
  ...
  for (const [name, value] of Object.entries({
    '--docus-bg': '#ffffff',
    '--docus-surface-1': '#f9fafb',
    '--docus-border': '#e5e7eb',
    '--docus-divider': '#e5e7eb',
    '--docus-text-1': '#111827',
    '--docus-text-2': '#4b5563',
    '--docus-text-3': '#6b7280',
    '--docus-accent-hover': '#4f46e5',
    '--docus-accent-pressed': '#4338ca',
    '--docus-radius-md': '8px',
    '--docus-font-size-md': '1rem',
    '--sans': 'system-ui',
    '--mono': 'ui-monospace',
...
describe('ToastHost Naive bridge', () => {
  it('maps info, success, and error without adding warning to the Docus API', async () => {
  ...
  it('keeps Docus TTL authority for defaults, custom TTL, and ttl zero', async () => {
    vi.useFakeTimers()
