...
// Eye — open eye (lens + iris). Used by the NavBar view-toggle to
// represent the "switch to read mode" target. Universally read as
// "observe / view", which is the correct verb for read mode (you're
// looking at the rendered document, not editing it).
...
// (Currently unused by the view-toggle, which uses ICON_EYE instead;
// kept here because the AI panel's read_file action description still
// references this silhouette in design notes.)
...
// ICON_AB_GIT_HISTORY — filled Git branch graph used by the History
// activity button. The supplied glyph is normalized to the shared
// 24×24 surface canvas and keeps the dirty-count badge separate.
