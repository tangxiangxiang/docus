...
import { isDocusShortcutBlocked } from '../keyboard'

describe('Docus shortcut boundary', () => {
  it.each(['input', 'textarea'])('blocks shortcuts from %s elements', (tagName) => {
    const element = document.createElement(tagName)
    document.body.appendChild(element)
    const event = new KeyboardEvent('keydown', { bubbles: true, cancelable: true })
    element.dispatchEvent(event)
    expect(isDocusShortcutBlocked(event)).toBe(true)
    element.remove()
  })
  ...
  it('blocks shortcuts from contenteditable and the explicit Excalidraw root', () => {
    ...
    expect(isDocusShortcutBlocked(contentEvent)).toBe(true)
    expect(isDocusShortcutBlocked(rootEvent)).toBe(true)
    root.remove()
  })
  ...
  it('allows shortcuts from ordinary Board chrome', () => {
    ...
    expect(isDocusShortcutBlocked(event)).toBe(false)
    button.remove()
  })
})
