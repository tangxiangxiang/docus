...
// directly:
//   - atomic replace replaces the target bytes and leaves no .docus-save-
//     intermediate behind
...
beforeEach(async () => {
  directory = await fs.mkdtemp(path.join(os.tmpdir(), 'docus-fs-platform-smoke-'))
...
describe('atomic replace (no POSIX mode assertion)', () => {
  it('replaces the bytes and leaves no .docus-save- residue', async () => {
...
describe('ownership-verified replace', () => {
  it('replaces when the expectation still holds and leaves no intermediates', async () => {
    ...
    expect(leftover.filter((name) => name.includes('.docus-save-')
      || name.includes('.docus-staged-')
      || name.includes('.docus-remove-'))).toEqual([])
  })
