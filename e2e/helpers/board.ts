...
export async function drawRectangle(page: Page): Promise<void> {
  ...
  // Excalidraw's accessible radio input intentionally sits beneath its SVG
  // icon. Clicking its public label preserves the real toolbar event path.
  await root.getByTestId('toolbar-rectangle').locator('..').click()
...
export async function insertImageThroughBrowserDrop(page: Page, png: Buffer): Promise<void> {
  const root = page.locator('[data-board-excalidraw-root]')
  const canvas = root.locator('canvas.excalidraw__canvas.interactive')
  await expect(canvas).toBeVisible()
  const box = await canvas.boundingBox()
  if (!box) throw new Error('Excalidraw interactive canvas has no layout box')
  await page.evaluate(({ base64, clientX, clientY }) => {
    ...
    const file = new File([bytes], 'board-b9.png', { type: 'image/png' })
