...
test('real Ledger onboarding and expense survive dashboard refresh', async ({ page, request }) => {
  ...
  const periodSummaryDate = page.getByTestId('ledger-period-summary-date')
  await expect(periodSummaryDate.locator('.ledger-period-summary-date-label')).toBeVisible()
  await expect(periodSummaryDate.locator('.ledger-period-summary-date-editor')).toHaveCSS('opacity', '0')
  await periodSummaryDate.hover()
  await expect(periodSummaryDate.locator('.ledger-period-summary-date-editor')).toHaveCSS('opacity', '1')
...
test('historical period navigation keeps one anchor across periods, reload, and browser history', async ({ page, request }) => {
  ...
  await page.getByTestId('ledger-period-summary-date').hover()
