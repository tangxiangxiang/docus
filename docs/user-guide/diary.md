...
Diary is Docus's date-based workspace. Use the Calendar to find a day, then
open that date in the same reading and editing experience used by ordinary
Docus documents.
...
Opening a date takes you to a native Docus document tab. The Diary document
uses Docus's normal Read and Edit surfaces, save behavior, and workspace tab
controls.
...
Unsaved changes continue to use Docus's existing close confirmation/save policy.
...
Diary content is a normal Docus Markdown document. Use Read mode to read it and
Edit mode to change it. Docus's existing save, dirty-state, conflict, and
...
Diary has a separate access password in addition to the Docus owner login.
The first visit asks you to set it up; later visits ask you to unlock Diary.
Locking Diary is available from the Docus navigation controls. Locking or an
expired session removes access to protected Diary bodies until you unlock
again.

If Docus finds legacy Diary data, the Settings migration section shows an
explicit scan and the required next action. Depending on the platform, Docus
may prepare an encrypted candidate for you to replace manually and then ask
you to resume verification. Do not delete the original until Docus reports
