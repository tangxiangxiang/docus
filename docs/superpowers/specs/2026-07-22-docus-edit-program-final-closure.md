...
- A ninth review round closed recovery-during-recovery and multi-file rename gaps: legacy quarantine identity manifests are now parsed and replayed by documentId CAS; explicit interrupted delete detaches metadata before removing the last staged bytes; replace quarantine uses a durable pending phase that recovers whether the old or new payload name survived; document and folder rename keep a second durable reference-transaction journal plus fsynced before/after payloads until every backlink is committed, allowing startup to roll forward only hash-matching references and stop on third-party content; and folder moves fsync their parent before metadata commit while source-side recovery reverses an already-moved metadata prefix. Closure remains reopened; this is review evidence, not a new seal.
- A tenth review round hardened the rename-reference transaction itself: document journals now require both documentId and source content hash, and replay verifies the destination file generation as well as its SQLite identity before touching backlinks; recovery settles main file/folder rename journals before independently named reference journals; reference journals are created first in a durable `preparing` phase, declare every payload name, then transition through `roll-forward`, `roll-back`, and `cleanup`, so crashes during preparation, compensation, or cleanup remain replayable without leaking bodies or deleting the only evidence; REST, AI, and folder rename persist rollback direction before undoing any reference, retain evidence whenever compensation is incomplete, and switch back to forward replay when source-path reuse leaves the owned generation at the destination. Closure remains reopened; this is review evidence, not a new seal.
- An eleventh review round closed the Windows counterexample and hardened the crash-test harness itself: (1) Windows folder moves are now a replayable create-only directory move — the journal is written first (mkdir gate, enumeration of regular files only with every entry's content hash; any symlink/junction/special file fails closed with a typed 501), files then move one by one via create-only `link(2)` under relative paths, an end-to-end parity check runs before the source is pruned, and a crash at any point leaves a SPLIT tree that startup recovery always completes forward from the journal — replacing the Windows `rename()`-onto-existing-directory EPERM failure with replayable semantics; (2) crash tests now prove the kill via a parent-child handshake — the child announces `READY:<point>` at its hook and hangs, the parent force-kills only after that line, and READY receipt plus a parent-initiated SIGKILL is the hard-kill evidence, with the exact crash-state on disk (split tree, journal phase, declared payload set) asserted BEFORE recovery runs — the old harness accepted any non-zero exit, which on Windows would have let a normally-completing child pass as "crashed"; (3) legacy metadata-less delete artifacts no longer write `{identities:[]}` quarantine manifests the parser rejects (an unparseable manifest would have blocked orphan cleanup forever); (4) journal provenance is now physically contained — an lstat ancestor walk strictly below the vault root plus a leaf lstat, so a vault-relative symlink (or Windows junction) whose target escapes the vault can no longer smuggle a reference path outside, and escape attempts quarantine with nothing written outside; (5) folder reference journals carry per-identity source hashes and recovery verifies them (or the self-reference after-hash), so external recreation at the destination quarantines instead of completing onto foreign bytes — legacy journals keep the weaker dev/ino proof; (6) the deterministic state machine now also randomizes replayable folder-move splits, folder-reference content proof, and legacy delete promotion (1100 new seeds, added to the three-platform CI crash bundle), and that model caught and fixed a real gap: the all-entries-at-source recovery branch skipped the destination→source metadata rollback the destination-absent branch performed; both sub-cases are unified behind one provably-ours gate with a single metadata reconciliation. The bound three-platform CI run then failed only on Windows and exposed a seventh, Windows-native production bug the local matrix could not see: the folder journal parsers required `Number.isSafeInteger` for dev/ino, but Windows volumes with large file records (NTFS extended IDs, ReFS/Dev Drive) report file IDs beyond 2**53, so every production folder journal on such a volume was unparseable and its recovery transaction could never complete — the parsers now accept any finite number (the values are only equality-compared against the same stat conversion, and the per-entry content hash remains the strong proof), with platform-independent regression tests forging beyond-2**53 IDs. The same run fixed two Windows-only test-harness gaps: the delete-rollback removal-injection mocks now match the staged directory's basename (the replayable protocol's private link-staging names live INSIDE the staged directory), and the external-content rollback test injects through the mkdir create-only gate both platforms share instead of a `rename` the replayable strategy never calls. Each fix carries mutation-verified regression tests (M11–M17). Closure remains reopened until the three-platform Actions run bound to this production SHA is green; this is review evidence, not a new seal.
- A twelfth review round closed the source-composition gaps the bound Windows-green run exposed: (1) **strategy schema drift (P0)** — the route persisted the runtime enum (`atomic-rename`/`replayable-move`) while the recovery parser only accepted `atomic`/`replayable`, so every real folder-rename journal was unparseable after a crash; both sides now import one shared `FolderMoveJournalStrategy` type, the parser accepts the canonical values (normalizing the legacy short names), and a tying test binds the parser to the platform default so they cannot drift again; (2) **journals now cover every physical file (P1)** — `listPhysicalMoveEntries` records every regular file the mover touches (buffer-hashed, so binaries are covered) with `documentId`/`documentPath` attached only to markdown documents, and empty trees carry an explicit `emptyTree` flag with a legal recovery path instead of an unparseable empty entries array; (3) **every replayable reverse move is now journaled (P1)** — rename rollback durably flips the main journal's direction before the first reverse file moves, reference-recovery rollback and delete rollback each write their own folder-move journal first (the delete journal persists the metadata snapshot so recovery re-installs the full graph), the delete-inflight orphan rule stands down while its companion journal exists, and new kill-point crash children prove each: rename rollback killed after the first reverse file, mid reverse move, and after the whole tree is back but before metadata, delete rollback killed mid restore, and recovery itself killed mid rollback replay; (4) **self-reference generation proof (P1)** — an internal backlink file is looked up by its destination identity path, so an already-rewritten internal document is recognized as ours instead of being quarantined as an external generation; (5) **mixed hash coverage rejected (P2)** — a folder-reference journal with some hashed and some unhashed identities no longer parses, so a stripped hash cannot downgrade the whole tree to the weak dev/ino proof. The crash child now drives the real HTTP route rather than hand-copying journal JSON, so the fixture cannot drift from production. Fixes carry mutation checks M18–M25, each verified RED then restored. Closure remains reopened until the three-platform Actions run bound to this production SHA is green; this is review evidence, not a new seal.
- A thirteenth review round closed six closure blockers in the v2 folder-move journal (source-composition audit), each mutation-verified (M26–M32): (1) **snapshot-restore trust boundary (P0/P1)** — a persisted delete-rollback snapshot is now scoped at parse time: every path must sit inside the restored folder's subtree, `documents[].path`/`id` cross-reference the declared paths/ids exactly, `document_tags`/`embeddings` reference only those ids, `tags` match the declared tagIds, `migrations` reference only the transaction's paths/ids, and every row carries exactly its table's columns — so a forged journal can no longer delete or replace unrelated metadata (a malicious-journal test proves `unrelated/*` rows are untouched); `preexistingTagIds` is recomputed against the live DB at restore so the orphan-tag cleanup cannot be weaponized; (2) **nested symlink/junction containment (P0/P1)** — recovery containment-checks every folder-move entry's source AND destination path (lstat walk of existing ancestors, rejecting any symlink — Windows junctions report as symlinks) before hash/mkdir/rename/link, so a symlinked subdir planted after the journal was written cannot route a touch outside the vault (the check is shared with the mover and skips not-yet-created ancestors, which are made as real directories); (3) **destination generation merge (P1)** — recovery inventories the destination before replaying: every present file must be a hash-matched landed entry, every directory a declared one, the only other allowed entry the mover's hidden gate token; any undeclared file/symlink/special entry quarantines instead of merging, and an otherwise-empty destination is provably ours only via its gate token, never by emptiness alone; (4) **nested empty directories (P1)** — the journal now records `directories[]` (every subdir including empty ones) and the mover + recovery recreate and parity-check the set, so empty directory nodes survive Windows replayable moves; (5) **reverse-move durability (P1)** — for the replayable protocol the journal direction flip is a hard precondition of the reverse move: if it cannot be persisted, not one file moves (the tree stays forward-consistent with both journals preserved for recovery), while atomic moves — a single rename, never split — still proceed; (6) **recovery pass bound (P1/P2)** — `recoverInterruptedOperations` loops until a pass makes no progress, capped by the startup artifact count plus headroom (hard ceiling 64), so arbitrarily deep crash-dependency chains (inner `.docus-rename-*` staging → companion folder-move journal → reference journal) close in one startup instead of stranding behind a fixed two-pass scan (a three-layer chain test proves it). The bound CI runs additionally exposed platform-specific test-harness issues (no production-behavior change): a `noUnusedLocals` build break from the rewrite's dead helper (removed); the multi-pass loop's separate pre-scan walk was the dominant cost on slow Windows fs (folded into the first pass); the deterministic models compared trees in NTFS-unstable readdir order, tore down with a Windows-transient `fs.rm`, and ran to the 120s ceiling (sorted comparison, a retrying teardown, 240s ceilings); a cold-cache flake in an unrelated markdown-render smoke test (stabilized to wait on the render condition); slow real-git tests hitting the 5s default on Windows (explicit ceilings, matching their siblings); and a draft-store IndexedDB blocked-open test whose late version-change resume stalls under CI load (bounded wait with the deletion assertion as arbiter — a real leak still fails). Closure remains reopened until the three-platform Actions run bound to this production SHA is green; this is review evidence, not a new seal.

## Round-17 Durable Metadata Intent

**Status:** Reopened. **Current closure commit:** not assigned. The implementation evidence below is not a seal; final Ubuntu, macOS, Windows, and Visual evidence must still bind to one final SHA.

Round-17 makes the journal-declared metadata transaction identical to the transaction executed by the route and independently replayed by Startup Recovery:

- A reverse `snapshot-restore` persists the complete rollback graph as `snapshot`, the exact overwrite precondition as `expectedCurrentSnapshot`, and only the physically moved document identities as `physicalDocumentIds`. Reference documents, destination metadata, tags, `document_tags`, embeddings, migration-only rows, and tombstones may therefore be metadata-only members of the rollback footprint without needing forged physical entries.
- Route rollback and Startup Recovery both read that durable disposition and call the same `restoreDocumentMetadataMutationCAS`. Ownership capture, canonical exact comparison, and restore execute inside one SQLite `BEGIN IMMEDIATE` transaction. A mismatch returns `snapshot metadata CAS failed`, retains the files-landed journal, and never overwrites the external transaction.
- Prefix journals persist `preparedSnapshot` before physical work and the actual `committedSnapshot` after the SQLite prefix transaction. Final cleanup compares the exact live graph across `documents`, `tags`, `document_tags`, `document_embeddings`, and `metadata_migrations`; a legacy metadata-committed prefix journal without an exact snapshot is parsed but quarantined.
- Prefix mutation plans documents and migrations before its first update. It moves migration-only `path`/`original_path` values, leaves `@deleted/<id>` tombstone paths unchanged while moving their `original_path`, rejects destination collisions transactionally, and uses an internal two-stage temporary namespace so primary-key swaps cannot leave partial rows.
- Safe reverse contention saves an immutable copy of the real forward metadata-committed journal. It restores that forward journal only when no reverse entry may have landed, destination generation and exact physical parity still match, the committed metadata snapshot still matches, and the original source is externally occupied. Replayable moves restore the exact gate marker proof before the journal rewrite. Partial reverse landing always retains the reverse direction.

The Round-17 crash matrix covers: forward prepared/gate-created/files-landed seams in the existing v4 suite; metadata committed before durable rewrite; reverse prepared full intent; reverse entry splits; reverse physical completion before CAS; reverse metadata before journal rewrite; source reuse before any reverse entry; source reuse after partial reverse landing; migration collision rollback; and second-recovery idempotency. The dedicated `round17FolderMoveDurableMetadataClosure.test.ts` additionally records the old 11/11 RED failures and the repaired 13/13 exact-intent matrix.

Local evidence at documentation time:

| Verification | Result |
| ------------ | ------ |
| `round17FolderMoveDurableMetadataClosure.test.ts` | 13 passed |
| `crashRecovery.test.ts` | 128 passed |
| Round-14/15/16 + `folders.test.ts` + metadata repository | 66 passed, 2 native-platform skips |
