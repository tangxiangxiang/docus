...
状态：`REVIEW-CLOSED`（VCalendar `3.1.2` exact-stack gate = `PASS / REVIEW-CLOSED`；D3.1 adapter 已完成但 Calendar-first surface、Vault integration 和完整 Diary lifecycle 尚未完成）
...
| `src/components/`、`src/views/`、`src/composables/` | D3.1 已有 `DiaryCalendar.vue` presentation adapter；没有 Diary editor 或 Calendar-first surface | 后续只接 D3.2 surface，不复制编辑器生命周期 |
| `src/content/` | 运行时 `CONTENT_DIR` 由统一 startup seed 确保包含 `diary/`；不是依赖静态 content fixture | `diary/` 已纳入固定根目录初始化，dev/prod 复用同一 helper |
| `package.json` / `package-lock.json` | D3.0 已按 exact registry resolution pin `v-calendar@3.1.2` 与 `@popperjs/core@2.11.8`；没有 Schedule-X；Docus resolved stack 为 Vue 3.5.35、Vite 8.0.16、TypeScript 6.0.3 | 兼容性依赖已获 D3.0 gate PASS；D3.1 adapter 已完成，Vault/UI/Editor integration 仍留在 D3.2+ |
...
- **D3.1 — Calendar adapter integration**：已使用 approved exact candidate 建立 `DiaryCalendar` presentation adapter；implementation commit 为 `931828da31166d68cb7897343c695a761bf6fc80`，adapter contract validation PASS，当前为 `COMPLETE / REVIEW-READY`，独立 review pending。
...
以下 checkbox 是 D3.0 gate evidence；D3.2/后续 production integration 仍未开始：
...
- [x] D3.1 production presentation adapter、VCalendar isolation、local DiaryDate adapter、marker/navigation/accessibility contract 和 component tests 已实现并验证；D3.1 = `COMPLETE / REVIEW-READY`，独立 review pending。
