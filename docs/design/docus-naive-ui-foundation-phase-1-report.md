# Docus Naive UI Foundation — Phase 1 Report
...
- `src/ui/tokens.css` is the global Docus semantic token authority.
- `src/ui/naiveTheme.ts` maps Docus tokens to the Naive UI theme boundary.
- `src/ui/DocusUiRoot.vue` owns the production provider hierarchy.
- `src/main.ts` mounts `DocusUiRoot` and keeps the required import order.
...
`style.css`. Status values use the smallest existing Docus mappings: settings
...
proved that `var(--docus-accent)` reaches an invalid-color path for `NInput`.
...
the explicit light/dark `--docus-accent` tokens.
...
Naive owns focus for those primitives; native/Docus controls continue to use
the broad Docus `:focus-visible` rule. The browser regression now checks the
...
the existing Docus appearance. Intentional visual changes: none. Regressions
