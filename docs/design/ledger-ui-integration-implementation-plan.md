...
所有 Ledger views 通过这个 store 的 actions 读取或刷新，不各自维护一份 account/overview/transaction copy。store 必须有显式 `reset()`，在 session expiry、logout、workspace unmount 或新 owner session 开始时清除上一个 owner 的财务 presentation state。
...
BOOTSTRAPPING
    ...
    ├── GET settings + no active/known Account → NO_ACCOUNT
    ├── GET settings + archived-only Accounts → NO_ACTIVE_ACCOUNT
    └── GET settings + active Account → READY
...
- no Account：转向 first Account CTA，不打开不可保存的 Sheet；
- archived-only：转向 restore Account；
...
当前 prototype 的“选择账本”、Bills terminology、mock account accents、budget-style class 和没有 period context 的 donut semantics 删除。Category breakdown / trend 保留真实信息目标，但具体图表形式以 Product Review 的视觉确认落地；不影响 API mapping 或 accessible data contract。

### 13.3 Empty and refresh behavior

- uninitialized/no-account 不请求并渲染正常 Dashboard shell；
...
- create key 在 retry 中保持不变、payload 改变后换 key；
- 401 / 404 / 409 / 503 / malformed body；
- money exponent / precision / overflow；
- Asia/Shanghai 与 DST timezone roundtrip；
- store reset、stale response epoch、no-mock invariant。
...
- implement uninitialized/no-account/zero-transaction/loading/error states。
...
实际编码阶段建议按以下 logical commits 拆分；本轮不创建 commit：
...
**None found in the audited L0 API.** 现有 API、shared DTO、currency metadata、projection、lifecycle、idempotency 和 owner auth 已覆盖本 Plan 的技术主轴。
...
不是 `Accepted`，也不是 `Ready for Implementation`。如果 Product Review 修改 onboarding、Dashboard、Category quick create、edit/delete、Account management 或 route migration 的决定，必须重新审计并更新本 Plan；不得在实现阶段自行做产品决定。
...
- [ ] exact test files、fresh DB real-data E2E；
- [ ] verification ladder、CI matrix 和 commit sequence。

本轮完成的文件范围以 §26 为准；不得因 Plan 生成而开始生产代码、修改 router、更新 package script 或执行 UI migration。
