# Docus Naive UI Foundation — Phase 2 Report
...
- `src/components/ToastHost.vue` bridges Docus `useToast()` to Naive
  `useMessage()`.
- `src/components/ConfirmHost.vue` renders the Docus confirm queue through
  declarative Naive `NModal` with the `preset="dialog"` primitive.
- `src/components/PromptHost.vue` renders the Docus prompt queue through
...
queue, Promise, cancellation, focus, and transform semantics stay under Docus
authority.
...
`ToastHost` keeps the Docus queue and TTL as the semantic authority. Each Docus
...
The Docus API remains intentionally limited to `info`, `success`, and `error`;
...
`useTheme()` and `useI18n()` remain the only Docus authorities; Naive theme and
