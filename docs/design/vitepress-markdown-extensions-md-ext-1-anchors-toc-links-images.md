...
The suffix is recognized only at the end of a heading's inline text. It is removed
from visible heading text while preceding strong/emphasis/code/emoji/link tokens are
preserved. Class, style, event, empty, whitespace, and other generic attribute
...
data-attribute restrictions are unchanged.
...
| Generated external target | target was configured but `_blank` was removed by the runtime URI check | only renderer-owned `_blank` is force-kept |
...
The generated token receives the renderer-owned docus-external-link class. Internal
.md, relative, fragment, WikiLink, mailto:, and tel: links are not external policy
targets. Raw semantic HTML anchors do not pass through this renderer and keep the
existing sanitizer behavior.

DOMPurify already allowed target in the configuration but treats _blank as a
URI-valued attribute in this runtime. A narrow sanitizer hook preserves only
target="_blank" on the renderer-owned docus-external-link class. It does not allow
arbitrary target values or alter raw HTML anchor policy. FORBID_ATTR: ['style']
remains exact.
...
| src/lib/markdown.ts | Anchor/TOC registration, sanitizer delta, Markdown image renderer |
