...
```text
[seemly/rgba]: Invalid color value var(--docus-accent)
```
...
Naive's derived alpha calculation; no full Docus token mirror.
...
- NButton: Docus `2px solid` outline plus Naive `1px` focus border — double
  indicator confirmed.
- NInput: Docus outline is suppressed by the Naive input primitive; Naive owns
...
`.n-button` and `.n-input input` from the broad Docus focus rule, leaving Naive
