...
- All twelve migration action templates use `NButton` with explicit `attr-type="button"`, preserved labels, density, disabled/working behavior, and existing Docus classes.
...
- Light and dark themes remain driven by the existing Docus theme authority.
...
- `docs/design/docus-naive-ui-foundation-phase5-diary-validation-report.md`
