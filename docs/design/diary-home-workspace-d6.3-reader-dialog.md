...
Implementation commit: `d270ee5756c0f742e92955f06fa308fd6f77bc4a`
...
command returns. The Reader receives the existing backing tab's `raw`, logical
path, loading/error state, and the existing `wikiResolver`. It does not call
`fetch`, `getPost`, `openPost`, a Diary create endpoint, or router navigation.
...
auto-open Reader from `activePath` or route watchers.
...
The focused Playwright command completed **14/14 PASS**:
...
- Focused Vitest: **8 files, 110 tests PASS**.
- `npm run typecheck:client`: PASS.
- `npm run typecheck`: PASS (client and server).
- `npm run build`: PASS.
- Focused Playwright Diary suites: **14/14 PASS**.
...
The D6.3 runtime rollback is the implementation commit
`d270ee5756c0f742e92955f06fa308fd6f77bc4a`. Documentation is informational and
is not part of the runtime rollback chain. No earlier D6.0–D6.2.1 commits need
to be reverted to remove this Reader adapter.
