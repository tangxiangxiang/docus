...
Diary is a date-based navigation workspace, not a second editor. Calendar
navigation finds a day; Vault owns the document that represents that day.
...
no `v-calendar` package or production import. The Docus integration is kept at
...
- `DiaryCalendar.vue` owns Docus date buttons, month display, weekday labels,
  mood controls, Today/Selected presentation, and Docus-owned ARIA/data
  attributes.
- Naive UI owns the Calendar panel and its month navigation primitives;
  Docus annotates the rendered previous/next controls with stable labels and
  test attributes.

Month navigation uses the provider's panel-change boundary and preserves the
current `DiaryCalendarMonth`. The weekday row is Docus-owned and follows the
...
Docus owner login. `App.vue` requests access before entering Diary, and the
...
platforms requiring user finalization, Docus gives the user the replacement
